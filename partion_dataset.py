...
if __name__== '__main__':
  ...
  parser.add_argument('-o', '--outputDir',help='Path to the output folder where the train and test dirs should be created. ''Defaults to the same directory as IMAGEDIR.',type=str,default=None)
  parser.add_argument('-r', '--ratio',help='The ratio of the number of test images over the total number of images. The default is 0.1.',default=0.1,type=float)
  parser.add_argument('-x', '--xml',help='Set this flag if you want the xml annotation files to be processed and copied over.',action='store_true')
  args = parser.parse_args()
  if args.outputDir is None:
    if not os.path.exists('/content/CCCD/TrainValDataset'):
      os.makedirs('/content/CCCD/TrainValDataset')
    args.outputDir = '/content/CCCD/TrainValDataset'

    # Now we are ready to start the iteration
  iterate_dir('/content/CCCD/Dataset', '/content/CCCD/TrainValDataset', 0.1, 1)
